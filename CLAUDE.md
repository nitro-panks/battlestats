...
Background enrichment runs on the Celery `background` worker via `enrich_player_data_task`, self-chaining between batches and kickstarted every 15 min by Beat (`player-enrichment-kickstart`). A daily DB-only `enrichment_pool_maintenance_task` (`enrichment-pool-maintenance`, 08:17 UTC, **coexists with crawls**) keeps the `pending` pool complete: re-queues `empty` false-negatives with a per-row cooldown guard (`ENRICHMENT_EMPTY_RETRY_AFTER_DAYS`) + an **incremental** per-realm `reclassify_enrichment_status --recent-hours 25` (skipped_* drift rescue scoped to recently-fetched rows via the `player_last_fetch_idx` index — ~2.5 min/realm). Kill switch `ENRICHMENT_POOL_MAINTENANCE_ENABLED`. The full-catalog reclassify (one-time backlog + pure-calendar inactivity drift) stays a **supervised manual op** (~36 min/run). Runbook: `agents/runbooks/runbook-enrichment-pool-maintenance-2026-06-09.md`.
