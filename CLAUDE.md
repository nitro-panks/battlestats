...
Background enrichment runs on the Celery `background` worker via `enrich_player_data_task`, self-chaining between batches and kickstarted every 15 min by Beat (`player-enrichment-kickstart`). A daily DB-only `enrichment_pool_maintenance_task` (`enrichment-pool-maintenance`, 08:17 UTC, index-backed + **coexists with crawls**) re-queues `empty` enrichment false-negatives into `pending` with a per-row cooldown convergence guard (`ENRICHMENT_EMPTY_RETRY_AFTER_DAYS`); kill switch `ENRICHMENT_POOL_MAINTENANCE_ENABLED`. The heavier full-catalog `reclassify_enrichment_status` (skipped_* drift) is a **supervised manual op** (~36 min/run on the 1-vCPU PG), not scheduled. Runbook: `agents/runbooks/runbook-enrichment-pool-maintenance-2026-06-09.md`.
