...
Five queues with dedicated workers: **default** (`-c 3`, light API refreshes), **hydration** (`-c 5`, request-driven upstream refreshes), **background** (`-c 3`, warmers/incrementals/snapshots/enrichment), **floor** (`-c 1`, the observation-floor capture cycles — recency-first, random-only, Beat-only), **crawls** (`-c 1`, the multi-day clan crawl + watchdog only).
