            ...
            {badges.map((b) => {
                ...
                return (
                    <Link
                        ...
                        title={`#${b.rank} in ${b.ship_name} over the last ${b.window_days} days — ${b.win_rate.toFixed(1)}% win rate`}
                        className="flex h-16 items-center gap-4 rounded-md border border-[var(--accent-faint)] bg-[var(--bg-card)] px-4 transition-colors hover:border-[var(--accent-mid)]"
                    >
                        <FontAwesomeIcon icon={faMedal} className={`${color} shrink-0 text-2xl`} aria-hidden="true" />
