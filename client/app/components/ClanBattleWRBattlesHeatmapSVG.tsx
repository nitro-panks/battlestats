...
// The clan-battle analog of RankedWRBattlesHeatmapSVG: same payload shape,
// scales, and drawing (reusing the shared rankedHeatmapPayload helpers); only
// the metric, endpoint, and copy differ. The population + player point come
// from PlayerExplorerSummary CB fields server-side (win rate already a percent).
interface ClanBattleWRBattlesHeatmapSVGProps {
    playerId: number;
    isLoading?: boolean;
    svgWidth?: number;
    svgHeight?: number;
...
interface ClanBattleWRBattlesPayload {
...
const drawChart = (containerElement: HTMLDivElement, payload: ClanBattleWRBattlesPayload, svgWidth: number, svgHeight: number, colors: Colors, theme: ChartTheme) => {
    if (!payload.tiles.length) {
        drawSvgMessage(containerElement, 'No clan battle population data available.', { width: svgWidth, height: 120, color: colors.labelMuted });
        return;
    }
    ...
    svg.selectAll('.cb-heat-tile')
        .data(payload.tiles)
        .enter()
        .append('rect')
        .attr('class', 'cb-heat-tile')
    ...
    // y-axis, matching the ranked heatmap's Courier stat treatment.
    ...
    if (payload.player_point) {
        ...
        summary.append('text')
            ...
            .style('font-family', "'Courier New', Courier, monospace")
            .style('fill', colors.labelMuted)
            .text('No clan battle history for this player');
    }
};

const ClanBattleWRBattlesHeatmapSVG: React.FC<ClanBattleWRBattlesHeatmapSVGProps> = ({
    playerId,
    isLoading = false,
    svgWidth = 600,
    svgHeight = 276,
    ...
    useEffect(() => {
        ...
        let isMounted = true;
        let cachedPayload: ClanBattleWRBattlesPayload | null = null;
        ...
        const fetchAndDraw = async () => {
            try {
                const { data: payload } = await fetchSharedJson<ClanBattleWRBattlesPayload>(withRealm(`/api/fetch/player_correlation/clan_battle_wr_battles/${playerId}/`, realm), {
                ...
                // Hide only this heatmap when the player has no stored CB summary
                // point (e.g. not yet backfilled); the scatter/timeline/table on
                // the tab render independently.
                const hasClanBattleHistory = Boolean(payload.player_point);
                onVisibilityChange?.(hasClanBattleHistory);
                if (!hasClanBattleHistory) {
                ...
                drawSvgMessage(containerRef.current, 'Unable to load clan battle heatmap.', { width: resolveWidth(), height: 120, color: colors.labelMuted });
            }
        };
...
export default ClanBattleWRBattlesHeatmapSVG;
