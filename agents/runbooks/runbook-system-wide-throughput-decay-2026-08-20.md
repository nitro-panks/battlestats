...
## Leading hypothesis: the database

Two independent lines point here.
...
**2. The read-side warmers have now joined the decay.** These were flat through Aug 19 and are not flat on Aug 20:

| task | Aug 17 | Aug 19 | Aug 20 (partial day) |
|---|---|---|---|
| `snapshot_active_players_task` | 390.3s | 387.0s | **549.9s** |
| `warm_hot_entity_caches_task` | 166.3s | 165.7s | **267.2s** |

Pure-DB aggregation work slowing by 40% to 60% is the signature of the shared resource being the database rather than anything WG-shaped. It is also why the decay reaches both WG-bound tasks (recapture, floor) and read-only warmers.
...
- **`benchmarks/db-size/` is stale**, newest file 2026-06-22. It cannot answer the growth question.
...
2. **Sample the DB during the recapture window today**, 10:00 to 11:30 UTC, from the droplet, and compare against the 04:41 numbers. This separates "the DB is saturated all day" from "the 04:30 rollup saturates it briefly."
3. **Rank `pg_stat_statements` by `shared_blks_read` and `total_exec_time`.** Do not reset it on prod (blocked by the auto-mode classifier); snapshot and diff over a window.
