...
2. **F5 observation row retention** — the next lever. Extend the twice-monthly archive job: delete JSON-stripped rows older than the 32d window except each player's latest observation (floor-freshness anchor), and fully-empty rows past ~7d. Note the BattleEvent provenance FKs were already relaxed in migration 0082 (`db_constraint=False`, `on_delete=DO_NOTHING`), so observation row deletes are no longer FK-encumbered.
3. **F9.1 crawl candidate scans** (the single biggest DB consumer, ~31 s mean) — `EXPLAIN` the two anti-join statements; likely fix is a partial index matching the crawl predicate or a small maintained frontier table.
