...
4. **F9.2 `ship_pop_avg_damage` rollup** (per-(realm, ship, day) incremental), **F8** drop the `checkpoints*` tables after a reference check, **item 10** env-gate default alignment (`BATTLE_OBSERVATION_COMPACT_ENABLED`, `HOT_PLAYERS_ENABLED`). **F9.4** — DONE 2026-07-20 (see Applied log; work item `agents/work-items/f94-json-analytical-pass-fix.md`); verify the call-rate drop in `pg_stat_statements` after ~a week.
