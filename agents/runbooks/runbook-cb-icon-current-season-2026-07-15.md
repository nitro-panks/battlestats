...
The metadata Redis key is 24h-cached, and WG lists a new season on its own schedule. If a player's `clans/seasonstats/` rows contain a **regular** season id newer than the max known regular season, `fetch_player_clan_battle_seasons` busts the metadata key and refetches once (`force_refresh=True`) before joining — bounding rollover lag to WG's own listing latency, exactly like `update_ranked_data`'s ranked self-heal. The refetch is **skipped once that season is already in the durable `ClanBattleSeason` table** (published OR activity-imputed, below) — otherwise every CB fetch during a publish-lag gap re-hits `clans/season/` uselessly; real dates land via the normal 24h cache expiry once WG publishes.

### Activity-imputed rollover (WG publish-lag fallback, 2026-07-23)

`clans/season/` can lag the season it dates by a week or more: per-player CB stats return the new season's battles at open, but `clans/season/` still tops out at the prior season, so `get_current_clan_battle_season_id()` (newest *started* regular season by start_date) stays pinned to the prior season and the shield clings to last-season players. `_impute_clan_battle_season_from_activity(seasons, season_meta)` (called from `fetch_player_clan_battle_seasons` before resolution) writes an imputed `ClanBattleSeason` row for the **regular** season `max_known_regular + 1` with `start_date` = first observation when a fetched player has battles there and WG hasn't dated it. It also **injects the imputed season into the (private-copied) `season_meta`** so the per-season result rows carry the imputed `start_date`/`label` instead of nulls (v4.3.8) — the CB analogue of the ranked `ranked_json` row-stamp. Guards mirror the ranked path (`_impute_ranked_season_from_activity`): only the immediate next season; **only regular ladder ids** (`<= CLAN_BATTLE_REGULAR_SEASON_MAX_ID` = 99 — a brawl/special id 101+/201+/301+ can never become current); only once the prior season has ended (`end_date < today` or null); never bootstraps an empty reference; idempotent. Self-corrects: `_upsert_clan_battle_seasons_reference`'s unconditional `update_or_create` overwrites the imputed dates with WG's real ones once published, and imputation stops. This is the direct CB analogue of the ranked S29→S30 fix (v4.3.6); spec `agents/work-items/ranked-enjoyer-current-season-spec.md`.
