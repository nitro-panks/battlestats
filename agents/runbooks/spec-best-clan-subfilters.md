# Spec: Best Clan Sub-Filters (Overall, WR)
...
Add two sub-filter options under the "Best" clan mode on the landing page: **Overall** and **WR** (Win Rate). These should provide two backend-ranked views, each with its own top 25 clans, rather than client-side re-sorting of one shared top-25 payload.
...
Each sub-filter owns its own top-25 result set. The WR view must not be derived by reordering the same 25 clans returned for Overall.
...
**Implication:** There is still no clan-level league placement feed in the WG API. The remaining WR view can only use stored member-derived clan-battle aggregates as a qualified lift on top of overall clan WR.
...
Active Clans  [ Best ]  [ Random ]  [ Recent ]  (i)
               Overall · WR                               ← sub-filter row (visible on load)
```
...
Both sub-sorts should be produced by the backend. The client should render the returned order directly.

Why this is the required contract:

1. `Overall` and `WR` are meaningfully different ranking questions.
...
3. Each sort can have its own cache entry per realm.
...
2. Build dedicated ranking helpers for `overall` and `wr`
...
3. Add `clanBestSort` state (`'overall' | 'wr'`, default `'overall'`)
...
7. Keep the client as a thin renderer; do not compute ranking locally
...
- `client/app/components/ClanTagGrid.tsx` — may need minor adjustment if row metadata changes

### Phase 3: Visual polish

1. Refine sub-filter link styling (underline weight, spacing, transition)
2. Consider whether the WR row needs any additional explanatory copy or icon affordance
...
- **Frontend:** Playwright test that Overall and WR issue distinct backend requests and render returned order directly
...
- **Backend:** Tests that `overall` and `wr` each produce their own top-25 ranking from the full eligible pool
- **Backend:** Contract test that the endpoint accepts `sort` and preserves returned order
- **Edge cases:** Clans with no CB data, single-clan Best list, realm switch resets sub-filter, and cases where WR top 25 diverges materially from Overall
...
- Implementation note: the backend fix also corrected clan-metric aggregation to join `PlayerExplorerSummary` through `Clan.clan_id` instead of the clan table primary key, which was required for the WR ranking to operate on the intended clans.
...
| 2     | `client/app/components/ClanTagGrid.tsx`  | Optional row-level metadata follow-up                                         |
