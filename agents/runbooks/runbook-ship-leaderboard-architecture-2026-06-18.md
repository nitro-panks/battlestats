...
How the ship-standings feature works end to end: a nightly precompute writes a
...
Ship Honors removal), `runbook-ship-banner-ux-pass-2026-06-05.md`,
...
```
nightly task ──> compute (BattleEvent aggregate + shrinkage + z-score)
...
## 1. Nightly compute — the source of truth

- **Task:** `snapshot_ship_top_players_task` — `server/warships/tasks.py:1050`.
  Runs **per realm, twice daily** (every 12h, realm-striped in `signals.py`),
  gated by `SHIP_BADGE_SNAPSHOT_ENABLED`. Delegates to
  `data.compute_ship_top_player_snapshot()`; on success enqueues
  `materialize_landing_player_best_snapshots_task` (`tasks.py:1085`) to refresh
  landing caches.
- **Compute:** `data.compute_ship_top_player_snapshot()` — `data.py:6040`.
  1. **Aggregate `BattleEvent`** over a trailing `SHIP_LEADERBOARD_WINDOW_DAYS`
     window (default **14**, `data.py:5964`) — filtered `mode='random'`,
  ...
  3. **Within-pool z-score** of three signals — win rate, avg damage/battle,
     ...
     `_DAMAGE=0.35` / `_KILLS=0.15` (`data.py:6120`) into one composite.
...
while held, no durable ledger." A rolling ledger would inflate ~14× (rolling
decision, 2026-06-14), which is why nothing accumulates.

## 2. Model — `ShipTopPlayerSnapshot` (`models.py:785`)
...
- View `ship_leaderboard` — `views.py:2219`. Validates realm → Redis read-cache
  `{realm}:ship-lb:{ship_id}`, TTL `SHIP_LEADERBOARD_CACHE_TTL=900` (15 min,
  `data.py:5965`) → on miss `data.get_ship_leaderboard()` (`data.py:6457`).
...
## 4. Profile badges — `data.get_player_ship_badges` (`data.py:6326`)
...
leads). Bulk variant `get_players_ship_badges_bulk` (`data.py:6389`) does it in
...
Each night, per realm, the system rewrites a trailing-14-day **random-battles**
...
`SHIP_BADGE_SNAPSHOT_ENABLED` (kill switch) · `SHIP_LEADERBOARD_WINDOW_DAYS=14` ·
