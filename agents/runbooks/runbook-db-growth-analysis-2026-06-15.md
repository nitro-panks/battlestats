...
5. **[DONE — KILLED 2026-06-15] weekly/monthly/yearly rollups** — user decided KILL. The 3 period tables + all period-writer/reader code + the `BATTLE_HISTORY_PERIOD_ROLLUP_ENABLED` gate were removed (migrations `0074_drop_period_rollup_tables`; daily layer + `?window=`/`?days=` endpoint unchanged). This also **closes the nightly-rollup OOM follow-up** — there is no period rebuild left to time out. Disk reclaim (~1.18 GB) lands when the operator runs `migrate` in prod (`DROP TABLE` returns space to the OS; no `VACUUM` needed).
