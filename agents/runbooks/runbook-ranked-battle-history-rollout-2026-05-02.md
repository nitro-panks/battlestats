...
_Update (2026-07-13): the Phase 5 mode pill (`Random | Ranked | All`) was **removed** — 90d Umami showed 136 clicks / 35 sessions (~1.7% of player sessions). `BattleHistoryCard` now takes a fixed `mode` prop; the Activity tab renders `mode="random"` with a static "Random Battles" caption, and ranked battle history (sparkline included) relocated to a "Recent Ranked Battles" section on the Ranked insights tab. `combined` is no longer reachable from the UI; the `?mode=` API (this runbook's Phase 4) is unchanged. Everything below about the backend pipeline, season scoping, and baselines remains current._
