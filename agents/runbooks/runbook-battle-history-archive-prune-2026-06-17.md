# Runbook: Twice-monthly cold-archive + prune of battle-history (>105d retention)

_Created: 2026-06-17_
_Update 2026-09-20 (supersedes every "twice-monthly" / "1st + 15th" / "03:00 UTC" statement below, including the unit listing): **the timer fires DAILY at 07:00 UTC.** A delete-based prune returns nothing to the OS, so the two tables stay at their high-water mark forever: ~120 days of rows on the old cadence, ~106 daily — about 2.3 GB never allocated, decided before the first peak. 07:00 rather than 03:00 because a daily job must not share an hour with the standings warmers (02:30-03:25), the daily rollup (04:00, which rewrites the newest days of the same tables) or the reconcile (05:00). Pinned by `test_battle_history_archive_timer_fires_daily`. Full reasoning: `runbook-db-table-shape-remediation-2026-09-20.md` Step 1. The text below is left as written, as the record of the original design._
_Update 2026-08-05 (supersedes the 92d text below): **live retention is 105 days**, pinned in `server/deploy/deploy_to_droplet.sh:705` — read the deploy script, never a code default or this prose, for the live value. Raised 32 → 92 on 2026-07-20 (enabled by a DB disk resize 60 → 80 GiB, autoscale still OFF), then **92 → 105 on 2026-07-24** as Lever C of the 90d-window work. **105 is sized to sustain a 90-day rolling read** (the product target; 45d live and 60d are stepping stones, not a selector) — the twice-monthly prune cadence means a 90d read needs ~15d of slack plus safety, so the excess is the mechanism, and **retention reduction is not a disk-capacity lever**. The mechanism below is unchanged; read the historical "32d"/"92d" figures as earlier windows._
_Correction 2026-08-05: the window deepens forward-only from the **measured floor** `min(detected_at)` = **2026-06-13** — it was never emptied at the transition, it inherited 37 days of depth. Full 105d depth therefore lands **2026-09-26** (not the "~2026-09-18" a transition+N construction gives); first prune run with candidates 2026-10-01, first substantial one 2026-10-15; steady state oscillates **105–120 days**, and 120 is the figure that meets the disk wall. The old "~6.2 → ~18 GB, DB ~38 → ~50 GB" cost estimate **understated the plateau by ~14 GB**: it modelled only `BattleEvent` + `PlayerDailyShipStats` and omitted `warships_battleobservation` (the actual largest growth driver) and `warships_player`. Current projection: plateau ~67.5 GB ≈ 80% of the volume. See `agents/work-items/db-growth-capacity-2026-08-05.md`._
...
| Retention | **105 days** (32 until 2026-07-20; 92 until 2026-07-24), both tables | One window for everything in scope. Sized to sustain the 90d rolling read against a twice-monthly prune cadence. Authority: `deploy_to_droplet.sh:705`. |
| UI impact (accepted) | week/month/60d/year cap at 105 days (32 until 2026-07-20; 92 until 2026-07-24) | Permanent; the deeper window fills forward-only. |
...
- **`manifest.json`** (one per run-date, per table or combined): `table`, `cutoff` (ISO), `min_date`/`max_date` of exported rows, `rowcount`, `sha256` of the `.csv.gz`, `columns` (ordered column list — pins the CSV header against future schema drift), `app_version` (from root `VERSION`, resolved as `settings.BASE_DIR/../VERSION`; fallback `NEXT_PUBLIC_APP_VERSION`, else `"unknown"`). The deploy did not ship the root `VERSION` file until 2026-07-15, so manifests from the 2026-06-17 / 2026-07-01 / 2026-07-15 runs record `"unknown"`; `deploy_to_droplet.sh` now scp's `VERSION` to the release root and runs from 2026-08-01 onward stamp correctly.
