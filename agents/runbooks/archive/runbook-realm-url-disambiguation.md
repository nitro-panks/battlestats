...
1. In `RealmContext.tsx`, on initial mount read `window.location.search` for a `realm` param. If it's a valid realm (`na` | `eu`), use it as the initial value and persist to localStorage:
   ...
   const initial = urlRealm && VALID_REALMS.includes(urlRealm) ? urlRealm : storedRealm;
   ```
...
- Shift: `withRealm` was added independently to fetch functions before this runbook began, but frontend routes strictly use `buildPlayerPath/buildClanPath` as planned.
- Test suite failed due to an unmocked import of `ClanTierDistributionSVG` in `ClanDetail.tsx` resulting in ES Module translation failure - fixed via `jest.mock`.
