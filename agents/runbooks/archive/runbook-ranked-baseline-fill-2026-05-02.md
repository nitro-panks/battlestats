...
_Status: helper-and-command-shipped, awaiting on-droplet dry-run + live fill — 2026-05-02. `record_ranked_observation_and_diff` (3 WG calls) added next to `record_observation_and_diff`; `establish_ranked_baseline` management command lives in `server/warships/management/commands/`. 14 new tests cover the helper (3 cases) and the command (11 cases). Lean release gate green (241/241). 2026-05-04 update: production-side execution remains gated on operator-approval to SSH the droplet — capture seam is verified end-to-end live (1/10 top NA ranked players already showing rolled-up ranked rows; the other 9/10 are in the expected single-observation pre-fill state). Running the live fill at default `--min-ranked-battles 100` will convert those single-observation players into baseline-state and unlock ranked data for the rest of the eligible NA population._
