...
- `HOT_PLAYERS_ENABLED` (1) — master kill switch (`maintain_hot_players_task` + `capture_hot_player_observations_task` both no-op at 0). **Prod pins `0` (disabled 2026-06-16)** — live hot set was ~98.5% active-7d (already floor-covered), so the queue was near-pure floor overlap; pinned in the `deploy_to_droplet.sh` env block (survives the `.env.cloud` overwrite).
