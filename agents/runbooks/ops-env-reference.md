...
- `HOT_PLAYERS_FRESH_AFTER_MINUTES` (12) / `HOT_PLAYERS_FRESH_CYCLE_MINUTES` (12) — latency runbook **Tier 3**. A separate `refresh_hot_player_freshness_task` (per-realm striped, `background`, gated on `ENABLE_CRAWLER_SCHEDULES` + `HOT_PLAYERS_ENABLED`) sweeps every `_CYCLE` minutes and re-refreshes any hot player whose `battles_updated_at` is older than `_AFTER` minutes (skip-if-fresh) — calling `update_battle_data(force_refresh=True)` to bypass the 15-min cache guard — so durably-engaged players stay inside `PLAYER_BATTLE_DATA_STALE_AFTER` (15 min) and visits resolve sub-second with no live WG refresh. ~1 WG call/refresh, bounded by `HOT_PLAYERS_MAX`. Runbook: `runbook-player-refresh-latency-2026-06-10.md`.
...
- `SHIP_BADGE_MIN_BATTLES` (15), `SHIP_BADGE_MIN_SHIP_POPULATION` (20), `SHIP_BADGE_MIN_SHIP_POPULATION_CV` (10) — carrier-only population floor; CVs are a low-volume class (few players grind ≥`MIN_BATTLES` on a single CV per season), so the universal 20 leaves most T10 CVs off the standings, `SHIP_BADGE_LIST_SIZE` (15), `SHIP_BADGE_TOP_N` (3), `SHIP_BADGE_TIERS` (default `10`; prod pins `8,9,10`; legacy `SHIP_BADGE_TIER` fallback), `SHIP_BADGE_RETENTION_DAYS` (30)
