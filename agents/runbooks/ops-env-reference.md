...
- BattleObservation floor: `BATTLE_OBSERVATION_FLOOR_HOUR`/`_MINUTE` (1/15), `_HOURS` (8), `_LIMIT`/`_DELAY` (default 3000/0.3; **prod=7500** for R3 — normal-cycle candidate cap), `_CRAWL_DELAY`/`_CRAWL_LIMIT` (0.8 / default falls back to LIMIT; **prod=3000** to stay gentle on the 1-vCPU DB while a crawl holds the realm lock — floor coexists with crawls instead of skipping)
