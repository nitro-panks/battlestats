...
- BattleObservation floor — random-first routing: `BATTLE_OBSERVATION_FLOOR_RANDOM_FIRST_ENABLED` (0). Routes a player to the heavy 3-call ranked path only if they have ranked battles in a **currently-active** season (`Player.ranked_last_season_id`, refreshed ~2h by `update_ranked_data`); everyone else — incl. lapsed ranked players — takes the fast bulk-random path so Random coverage isn't throttled by a niche mode. Falls back to ever-ranked when season metadata is unavailable. Command: `--random-first`. Runbook: `runbook-bulk-battle-observation-capture-2026-06-06.md`. (Standing product rule: Random > Ranked.)
