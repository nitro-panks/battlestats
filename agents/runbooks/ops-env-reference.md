...
- `HOT_PLAYERS_FRESH_AFTER_MINUTES` (code default 12, **prod=1440 since 2026-06-13**) / `HOT_PLAYERS_FRESH_CYCLE_MINUTES` (12) — `refresh_hot_player_freshness_task` (per-realm striped, `background`, gated on `ENABLE_CRAWLER_SCHEDULES` + `HOT_PLAYERS_ENABLED`) sweeps every `_CYCLE` minutes (cheap, DB-only scan) and fires `update_battle_data(force_refresh=True)` on any hot player whose `battles_updated_at` is older than `_AFTER` minutes. **Prod intentionally gates this to once/24h** (`_AFTER=1440`): the tactical objective is ≥1 battle-history refresh per visited player per 24h, not perpetual sub-second freshness. The 12-min default was the latency runbook's **Tier 3** sub-second-on-visit optimization, **retired in prod 2026-06-13** — a visit to a hot player now >15 min stale falls back to the normal live-refresh-on-view path. ~1 WG call/refresh, bounded by `HOT_PLAYERS_MAX`. Runbooks: `runbook-hot-players-engagement-queue-2026-06-10.md`, `runbook-player-refresh-latency-2026-06-10.md`.
