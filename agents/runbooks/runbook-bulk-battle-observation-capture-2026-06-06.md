...
- `snapshot_movers` — distinct non-hidden **active-7d** players who battled since their previous stored row. **Redefined 2026-07-20** (delta-gated Snapshot writes, spec `agents/work-items/snapshot-delta-gated-writes-spec.md`): classified by the latest-date row's `interval_battles > 0` (the carry-forward delta vs the player's previous stored row — verified populated on every bulk row, 0 NULLs across 3 live days), falling back to the legacy two-date cumulative pair when the interval is NULL. The old claim that `interval_battles` was view-path-only was verified false. Under dense writes both definitions agree (~72K movers/day baseline), so the KPI series is continuous across the transition. Still scoped to active-7d and computed as two independent reads intersected in Python. **Measured live 2026-06-15: 16,766 / 67,255 movers captured = 24.9%** (snapshot_coverage_frac 100%) — the floor captures ~¼ of daily movers, the ~4× headroom the per-ship-daily goal must close.
...
- `snapshot_today` / `snapshot_coverage_frac` (÷ active-7d) — guards the denominator: if snapshot coverage is well under 100%, the mover denominator is itself incomplete and the snapshot engine (`SNAPSHOT_ACTIVE_LIMIT`) needs scaling before the KPI can be trusted. **Under delta-gated writes (`SNAPSHOT_DELTA_GATE_ENABLED=1`, the default since 2026-07-20) `snapshot_coverage_frac` is `null`** — only movers get rows, so "unchanged" and "unchecked" are indistinguishable from `Snapshot` alone; the denominator guard is instead the engine's own `Queued/Unchanged-skipped` counters walking the whole pool daily (checked-set convergence). `snapshot_today` then counts rows written on the latest date (≈ movers), not coverage.
...
- `no_snapshot_pair` — no classifiable snapshot signal (no latest-date row, or a NULL interval without a two-date pair); unclassifiable. Under delta-gated writes this bucket also absorbs active-1d players whose PvP stats didn't move (no row written) — expect it to grow relative to `non_pvp_active`, which now only counts players with an actual zero-interval today-row.
