...
- **Agent Directives:**
  ...
  3. **High-Timeout E2E Polling Checks:** Any automation testing async Celery pipelines _must_ use high timeouts (e.g., `60000ms` vs `30000ms`) inside Playwright `toBeVisible()` assertions to allow sufficient cold-start backpropagation across production droplets before falsely assuming the rendering failed.
