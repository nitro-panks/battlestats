...
| `battle-history-ships-scope` | `{scope:'all'\|'slider', count:<N>}` (3.3.0 slider; `top8`/`top10` strings in 3.2.5–3.2.8 captures) | Release the battles×dmg ships treemap zoom slider (once per release, not per drag tick) | `BattleHistoryTreemaps.tsx` (trackScopeRelease) | 🟡 deployed v3.3.0, awaiting captures |
