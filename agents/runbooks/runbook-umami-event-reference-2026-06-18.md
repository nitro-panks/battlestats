...
| `battle-history-ships-color` | `{metric:'wr'\|'dmg'\|'kills'}` | Click a color-metric pill (WR% \| dmg \| Kills) in the battles× treemap header — one shared control recolors all three battle-history maps; default `wr` | `BattleHistoryTreemaps.tsx` (chooseColorMetric) | 🟡 added 4.2.0, pending deploy+captures |
| `efficiency-sort` | `{realm, column:'name'\|'tier'\|'type'\|'award'\|'battles'\|'wr', direction:'asc'\|'desc'}` | Click a column header in the Efficiency badge table; `direction` is the resolved direction after the click | `EfficiencyBadgeTable.tsx` (onSort) | 🟡 added 4.3.x, pending deploy+captures |
| `efficiency-filter` | `{realm, control:'tier'\|'type'\|'award'\|'clear', value}` | Set an Efficiency filter — via a dropdown, a **treemap tile click** (toggles; `value` is the tile's value or `all` on toggle-off), or the **Clear button** (`control:'clear'`, `value:'all'`) | `EfficiencyBadgeTable.tsx` (applyFilter/clearFilters) | 🟡 added 4.3.x, pending deploy+captures |
...
| `ship-stats-close` | `{ship_id, source:'button'\|'row'\|'treemap'\|'backdrop'\|'escape', mode, window, realm}` | Close the ship-stats modal (✕ button, backdrop click, Escape, re-click the same row, or re-click the same treemap tile). `backdrop`/`escape` sources added when the panel became a modal overlay (2026-07-16) | `BattleHistoryCard.tsx` (toggleShip/closeShipStats/Escape effect) | ✅ live-verified 2026-06-18; `backdrop`/`escape` 🟡 pending captures |
...
- ~~**Efficiency medal force graph** (`EfficiencyStripPlotSVG.tsx`, v3.6.0) — hover-only; nothing to track.~~ **Superseded 4.3.0:** the Efficiency tab is now the sortable badge table (`EfficiencyBadgeTable.tsx`), whose sort-header and filter-dropdown clicks ARE tracked (`efficiency-sort` / `efficiency-filter` above). Tab entry is still captured by `player-insights-efficiency`.
