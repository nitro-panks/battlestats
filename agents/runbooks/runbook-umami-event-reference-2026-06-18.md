...
| `efficiency-filter` | `{realm, control:'tier'\|'type'\|'award', value}` | Change an Efficiency badge-table filter dropdown; `value` is the picked value or `all` when cleared | `EfficiencyBadgeTable.tsx` (onFilter) | 🟡 added 4.3.x, pending deploy+captures |
