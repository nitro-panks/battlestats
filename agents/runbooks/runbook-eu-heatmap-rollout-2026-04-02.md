...
This was implemented via `client/app/components/tierTypeHeatmapPayload.ts`.

## Validation

The implementation tranche that follows this runbook should not be considered complete until all of the following are true.

### Data validation

1. EU tracked population is non-trivial for each intended heatmap
2. EU heatmap warmers complete successfully without request-time WG calls
3. warm-cache payloads return populated tiles for EU where the realm has sufficient data

### Contract validation

1. backend tests cover the new compact payload shape if `tier_type` is flattened
2. client tests prove local reconstruction of compact payload geometry
3. realm-isolation coverage confirms `?realm=eu` stays on EU payloads

Current validation status:

1. focused Jest coverage passed for `PlayerDetailInsightsTabs`, `PlayerRouteViewWarmup`, `PlayerDetail`, and the new `tierTypeHeatmapPayload` helper test
2. backend view assertions were updated for the compact `tier_type` shape
3. local focused pytest could not be completed in this shell because Django test DB creation failed with `fe_sendauth: no password supplied`

### Performance validation

1. warm EU heatmap reads stay within the same general response class as NA warm reads
2. compact payload byte size is recorded before and after flattening
3. no new browser fan-out is introduced on player detail

## Files Expected To Change In The Follow-Up Tranche

Implemented in this tranche:

1. `server/warships/data.py`
2. `server/warships/serializers.py`
3. `server/warships/tests/test_views.py`
4. `client/app/components/TierTypeHeatmapSVG.tsx`
5. `client/app/components/playerProfileChartData.ts`
6. `client/app/components/tierTypeHeatmapPayload.ts`
7. focused client tests and e2e fixtures consuming `tier_type`

## Out Of Scope

This runbook does not include:

1. a new clan heatmap product
2. efficiency-based EU heatmaps that require waiting for Stage 2 enrichment
3. any new browser-triggered WG API reads
4. redesigning the existing heatmap visuals
5. changing unrelated landing scatter or histogram surfaces

## Rollback

If the follow-up implementation regresses payload compatibility or response times:

1. revert the compact tier-type payload version and serve the old shape from a bumped cache key
2. keep EU warming enabled for the existing ranked and survival payloads
3. leave the realm-aware API surface unchanged

The rollback should preserve the fact that EU is a first-class realm even if the new compact tier-type payload needs to be backed out.

## Recommended First Implementation Slice

The smallest safe vertical slice is:

1. measure current EU heatmap populations and payload sizes
2. flatten only the tier-type population payload
3. add focused backend and client tests for the compact shape
4. warm and probe EU heatmaps on the existing realm-scoped warm path

Do not combine this with new heatmap designs, new clan analytics, or enrichment backfill work.

## Follow-Up Notes

The remaining work is operational rather than contractual:

1. re-run focused backend pytest in an environment with working Postgres test credentials
2. capture explicit EU cold-build and warm-cache timings for `tier_type`, `win_rate_survival`, and `ranked_wr_battles`
3. only add published fallback behavior if those timings show a real cold-cache pain point on EU
