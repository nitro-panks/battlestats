#!/usr/bin/env bash
#
# Daily reproducible snapshot of the player-acquisition / battle-observation-floor
# benchmark. Wraps the read-only `benchmark_observation_floor` management command
# and writes a timestamped JSON file to a durable directory so progress can be
# measured day-over-day (e.g. baseline -> R3 floor-limit expansion -> after).
#
...
#   OUT_DIR       snapshot directory     (default $APP_ROOT/shared/benchmarks/observation-floor)
...
OUT_DIR="${OUT_DIR:-${APP_ROOT}/shared/benchmarks/observation-floor}"
KEEP="${KEEP:-180}"
...
"$VENV_PY" manage.py benchmark_observation_floor --json \
