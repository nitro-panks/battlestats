...
# Floor tuning env. SELF_CHAIN_ENABLED=1 (all realms): self-chain RE-ENABLED
# after the NameError warm-storm fix (5f7edd6) freed DB headroom; fills the -c2
# concurrency to continuously chew the backlogs; yields during crawls; watched by
# the managed-PG load monitor. (History: -c2 + self-chain was reverted earlier
# 2026-06-20 on DB saturation, but the root cause was a perpetual warm_landing
# re-warm STORM — a dangling LANDING_PLAYER_RANDOM_MIN_PVP_BATTLES ref in the
...
if grep -q '^SHIP_BADGE_TIERS=' /etc/battlestats-server.env; then
  sed -i 's|^SHIP_BADGE_TIERS=.*|SHIP_BADGE_TIERS=8,9,10,11|' /etc/battlestats-server.env
else
  echo 'SHIP_BADGE_TIERS=8,9,10,11' >> /etc/battlestats-server.env
fi
...
# Dedicated observation-floor worker: -c2 (2026-06-20). The floor is FETCH-bound in
# steady state — at -c1 a realm does only ~50 ships/stats/min while WG (9/s) and the
# DB sit idle — so a 2nd concurrent lane ~3x's mover capture (~150 ships/stats/min
# measured). The earlier -c1 choice was forced because mover-dense -c2 cycles + the
# heavy analytical WARMERS together saturated the 2-vCPU PG. THIS release cuts the
# warmer DB load (efficiency-rank event-triggers off + daily distributions/correlations
# — the prescribed "cut the analytical-warmer load" unlock), so -c2 now runs DB-safe
# (steady load15 settles ~1.5; the efficiency-rank neuter is immediate relief). Watched
# by the standing managed-PG load monitor; if it sustains load15>2.3, back off to -c1.
# -c3 never (saturates). Pairs with self-chain (-c2 fills both lanes) + gate-skip cooldown.
...
# `default`. Runs at -c 2 (self-chain ON, all realms) — two realms continuously
# chew their per-realm backlogs while a third queues. -c2 + self-chain was briefly
# reverted 2026-06-20 on DB saturation, but the root cause was a NameError warm
# storm (a perpetual warm_landing re-warm loop, fixed in 5f7edd6); with the DB
# recovered the -c2 + self-chain 2x-coverage goal was re-enabled, watched by the
# managed-PG load monitor. --time-limit=21600 (6h) mirrors background — a full
# realm sweep over a large stale pool is a multi-hour op. See
